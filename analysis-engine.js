...
export function loadAnalysisConfig(env = process.env) {
  const model = env.ANALYSIS_MODEL || 'gpt-4.1-mini';
  if (!PRICES[model]) throw new AnalysisError('CONFIG', 'Для выбранной модели нет проверенного тарифа. Используй ANALYSIS_MODEL=gpt-4.1-mini.');
  return {
    model,
    maxCostUSD: positive(env.ANALYSIS_MAX_COST_USD, 0.25),
    maxTotalTokens: positive(env.ANALYSIS_MAX_TOTAL_TOKENS, 120000, true),
    ...
    reportOutputTokens: positive(env.ANALYSIS_REPORT_OUTPUT_TOKENS, 4500, true),
    maxInputTokens: positive(env.ANALYSIS_MAX_INPUT_TOKENS, 12000, true),
    timeoutMs: positive(env.ANALYSIS_TIMEOUT_MS, 90000, true),
...
function renderReport(report, messages, stats, question) {
  ...
  const cite = item => {
    const evidence = item.evidence.map(({ id, quote }) => { const message = byId.get(id); return `  [${id}] ${message.date}, ${message.name}: «${quote}»`; }).join('\n');
  ...
  return {
    ...
    async analyze({ chatId, messages, question = '', ownerId, onProgress = () => {} } = {}) {
      ...
      try {
        ...
        const { chunks, parts } = makeChunks(records, cfg.chunkTokens);
        const coverage = { messages: records.length, textMessages: records.filter(message => message.text.trim()).length, characters: records.reduce((sum, message) => sum + Array.from(message.text).length, 0), parts, chunks: chunks.length, from: stats.from, to: stats.to };
        await privateDirectory(cfg.cacheDir);
        const directory = path.join(cfg.cacheDir, digest(String(chatId)));
        await privateDirectory(directory);
        const identity = { version: PROMPT_VERSION, model: cfg.model, question, ownerId: String(ownerId ?? ''), chunkTokens: cfg.chunkTokens, mapOutputTokens: cfg.mapOutputTokens, mergeOutputTokens: cfg.mergeOutputTokens, reportOutputTokens: cfg.reportOutputTokens, maxInputTokens: cfg.maxInputTokens };
        ...
        const root = createNode('report', records, level);
        ...
        const systemFor = node => node.kind === 'map' ? MAP_SYSTEM : node.kind === 'merge' ? MERGE_SYSTEM : FINAL_SYSTEM;
        ...
        const readPlan = async node => {
          ...
          if (entry?.version === PROMPT_VERSION && entry?.key === node.key && entry?.result && tokenCount(entry.result) <= node.limit + 128) {
        ...
        const estimate = node => {
          ...
          const input = node.kind === 'map' ? inputEstimate(systemFor(node), node.payload, node.kind) : inputEstimate(systemFor(node), { ownerQuestion: question, ...(node.kind === 'report' ? { statistics: stats } : {}), summaries: [] }, node.kind, senders) + node.children.reduce((sum, child) => sum + (child.cache ? tokenCount(child.cache.result) : child.limit + 128) + 16, 0);
        ...
        await onProgress({ stage: 'plan', total: projected.calls, completed: 0, cached, estimatedCost, estimatedTokens: projected.input + projected.output });
